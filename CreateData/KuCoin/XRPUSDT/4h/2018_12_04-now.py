...
def get_kucoin_klines(symbol="XRP-USDT", interval="4hour", start_str="2018-12-04", end_str=None):
...
def save_kucoin():
    ...
    if df.shape[0] < 100 or df.isnull().values.any():
        ...
        return
    path = "../../../../data/KuCoin/XRPUSDT/4h/2018_12_04-now.csv"
    os.makedirs(os.path.dirname(path), exist_ok=True)
    df.to_csv(path, index=False)
    print(f"KuCoin CSV сохранён: {path}")
    plot_if_possible(df, "XRPUSDT", "4h")
