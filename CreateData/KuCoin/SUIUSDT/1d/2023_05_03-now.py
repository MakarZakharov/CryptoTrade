...
def get_kucoin_klines(symbol="SUI-USDT", interval="1day", start_str="2023-05-03", end_str=None):
...
def save_kucoin():
    ...
    if df.shape[0] < 100 or df.isnull().values.any():
        ...
        return
    path = "../../../../data/KuCoin/SUIUSDT/1d/2023_05_03-now.csv"
    os.makedirs(os.path.dirname(path), exist_ok=True)
    df.to_csv(path, index=False)
    print(f"KuCoin CSV сохранён: {path}")
    plot_if_possible(df, "SUIUSDT", "1d")
