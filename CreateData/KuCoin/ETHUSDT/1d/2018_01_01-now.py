...
def get_kucoin_klines(symbol="ETH-USDT", interval="1day", start_str="2018-01-01", end_str=None):
...
def save_kucoin():
    ...
    if df.shape[0] < 100 or df.isnull().values.any():
        ...
        return
    path = "../../../../data/KuCoin/ETHUSDT/1d/2018_01_01-now.csv"
    os.makedirs(os.path.dirname(path), exist_ok=True)
    df.to_csv(path, index=False)
    print(f"KuCoin CSV сохранён: {path}")
    plot_if_possible(df, "ETHUSDT", "1d")
