...
def get_kucoin_klines(symbol="SOL-USDT", interval="1day", start_str="2021-08-04", end_str=None):
...
def save_kucoin():
    ...
    if df.shape[0] < 100 or df.isnull().values.any():
        ...
        return
    path = "../../../../data/KuCoin/SOLUSDT/1d/2021_08_04-now.csv"
    os.makedirs(os.path.dirname(path), exist_ok=True)
    df.to_csv(path, index=False)
    print(f"KuCoin CSV сохранён: {path}")
    plot_if_possible(df, "SOLUSDT", "1d")
