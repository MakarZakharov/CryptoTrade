...
def get_kucoin_klines(symbol="BNB-USDC", interval="4hour", start_str="2022-06-01", end_str=None):
...
def save_kucoin():
    ...
    if df.shape[0] < 100 or df.isnull().values.any():
        ...
        return
    path = "../../../../data/KuCoin/BNBUSDC/4h/2022_06_01-now.csv"
    os.makedirs(os.path.dirname(path), exist_ok=True)
    df.to_csv(path, index=False)
    print(f"KuCoin CSV сохранён: {path}")
    plot_if_possible(df, "BNBUSDC")
