...
def get_kucoin_klines(symbol="SUI-USDC", interval="1day", start_str="2025-02-20", end_str=None):
...
def save_kucoin():
    ...
    if df.shape[0] < 100 or df.isnull().values.any():
        ...
        return
    path = "../../../../data/KuCoin/SUIUSDC/1d/2025_02_20-now.csv"
    os.makedirs(os.path.dirname(path), exist_ok=True)
    df.to_csv(path, index=False)
    print(f"KuCoin CSV сохранён: {path}")
    plot_if_possible(df, "SUIUSDC", "1d")
