...
def get_kucoin_klines(symbol="XRP-USDC", interval="4hour", start_str="2021-01-26", end_str=None):
...
def save_kucoin():
    ...
    if df.shape[0] < 100 or df.isnull().values.any():
        ...
        return
    path = "../../../../data/KuCoin/XRPUSDC/4h/2021_01_26-now.csv"
    os.makedirs(os.path.dirname(path), exist_ok=True)
    df.to_csv(path, index=False)
    print(f"KuCoin CSV сохранён: {path}")
    plot_if_possible(df, "XRPUSDC", "4h")
