...
def get_kucoin_klines(symbol="ETH-USDC", interval="4hour", start_str="2018-10-27", end_str=None):
...
def save_kucoin():
    ...
    if df.shape[0] < 100 or df.isnull().values.any():
        ...
        return
    path = "../../../../data/KuCoin/ETHUSDC/4h/2018_10_27-now.csv"
    os.makedirs(os.path.dirname(path), exist_ok=True)
    df.to_csv(path, index=False)
    print(f"KuCoin CSV сохранён: {path}")
    plot_if_possible(df, "ETHUSDC", "4h")
