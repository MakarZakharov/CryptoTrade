...
def get_kucoin_klines(symbol="SOL-USDC", interval="1day", start_str="2022-05-23", end_str=None):
...
def save_kucoin():
    ...
    if df.shape[0] < 100 or df.isnull().values.any():
        ...
        return
    path = "../../../../data/KuCoin/SOLUSDC/1d/2022_05_23-now.csv"
    os.makedirs(os.path.dirname(path), exist_ok=True)
    df.to_csv(path, index=False)
    print(f"KuCoin CSV сохранён: {path}")
    plot_if_possible(df, "SOLUSDC", "1d")
