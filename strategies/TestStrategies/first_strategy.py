...
# ======================
#   1) СТРАТЕГИЯ: ВСЕГДА ТОРГУЕМ ПОЛНЫМ КАПИТАЛОМ
# ======================
class AlwaysTradeFullCapital(bt.Strategy):
    def next(self):
        cash = self.broker.getcash()
        price = self.data.close[0]
        size = cash / price  # весь капитал

        if self.position:
            # закрываем текущую позицию
            self.close()
        # сразу же открываем новую
        self.buy(size=size)
...
if __name__ == "__main__":
    ...
    # Добавляем стратегию
    cerebro.addstrategy(AlwaysTradeFullCapital)

    # Загружаем данные из вашего CSV (ISO-формат даты с T)
    ...
    cerebro.broker.set_coc(True)                   # выполнять ордера по закрытию бара

    # Анализаторы для метрик
