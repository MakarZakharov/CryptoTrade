"""
Linear model implementation for STAS_ML v2
"""
...
class LinearModel(BaseModel):
    ...
    def __init__(self, config: Config):
        ...
        self.logger = Logger("LinearModel")
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, 
        ...
        if self.is_classification():
            self.model = LogisticRegression(random_state=self.config.random_seed, max_iter=1000)
        else:
    ...
    def get_feature_importance(self) -> Optional[Dict[str, float]]:
        ...
        if hasattr(self.model, 'coef_'):
            ...
            if len(coef.shape) > 1:  # Multi-class classification
                coef = coef[0]  # Use first class coefficients
