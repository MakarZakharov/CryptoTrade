...
class TradingEnv(gym.Env):
    """
    Реалістичне середовище для навчання STAS_RL-агента торгівлі криптовалютними парами.
    Підтримує проскальзування, комісії, спред, часткове виконання та моделювання ліквідності.
    
    НОВІ ПОКРАЩЕННЯ:
    - Підтримка ПОРТФЕЛЬНОЇ КОНТИНУАЛЬНОСТІ між епізодами
    - Збереження розміру портфеля для справжнього compound зростання
    - Покращена система розрахунку винагород
    - Жорсткіший контроль ризиків
    """
    ...
    def __init__(self, config: TradingConfig):
        ...
        # НОВЕ: Система портфельної континуальності
        self.enable_portfolio_continuity = getattr(config, 'enable_portfolio_continuity', True)
        self.persistent_balance = self.initial_balance  # Баланс, що зберігається між епізодами
        self.persistent_crypto_balance = 0.0  # Криптопозиція, що зберігається між епізодами
        self.episode_count = 0  # Лічильник епізодів
        self.cumulative_portfolio_history = []  # Загальна історія портфеля через всі епізоди
    ...
    def _setup_spaces(self):
        ...
        # ВИПРАВЛЕНО: Action space для PPO з природним розподілом
        # PPO з std=1.65 може продукувати дії в широкому діапазоні
    ...
    def reset(self, seed=None, options=None) -> Tuple[np.ndarray, Dict]:
        """
        Скидання середовища з ПОРТФЕЛЬНОЮ КОНТИНУАЛЬНІСТЮ.
        
        НОВА ЛОГІКА:
        - Якщо enable_portfolio_continuity=True: портфель зберігається між епізодами
        - Якщо enable_portfolio_continuity=False: повний скид як раніше
        """
        ...
        if self.enable_portfolio_continuity and self.episode_count > 1:
            # ПОРТФЕЛЬНА КОНТИНУАЛЬНІСТЬ: зберігаємо баланс між епізодами
            print(f"🔄 Episode {self.episode_count}: Continuing with portfolio ${self.persistent_balance + self.persistent_crypto_balance * self._get_current_price():.2f}")
            
            self.balance = self.persistent_balance
            self.crypto_balance = self.persistent_crypto_balance
            
            # Зберігаємо cumulative статистики, скидаємо тільки епізодні
            episode_trades = self.total_trades
            episode_profitable = self.profitable_trades
            
            # НЕ скидаємо загальні торгові статистики для континуальності
            # self.total_trades залишається
            # self.profitable_trades залишається
            
        else:
            # СТАНДАРТНИЙ СКИД: початок нового навчання або вимкнена континуальність
            if self.episode_count == 1:
                print(f"🆕 Episode 1: Starting fresh with ${self.initial_balance:.2f}")
            ...
            # Скидаємо persistent значення
            self.persistent_balance = self.initial_balance
            self.persistent_crypto_balance = 0.0
        
        # Скидаємо епізодні метрики (але НЕ загальні при континуальності)
        ...
        # Скидання схеми винагород (з збереженням історії для континуальності)
        if hasattr(self, 'reward_scheme'):
            if not self.enable_portfolio_continuity or self.episode_count == 1:
                self.reward_scheme.reset()  # Повний скид тільки при першому епізоді
        
        return self._get_observation(), self._get_info()

    def _get_current_price(self) -> float:
        """Отримати поточну ціну для розрахунків."""
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        # ВИДАЛЕНО: Форсована експлорація через шум (PPO має власну експлорацію)
        ...
        # ВИПРАВЛЕННЯ: Значно м'якші умови для truncation
        portfolio_value = self._get_portfolio_value()
        # Тільки при катастрофічних втратах (-95% або менше $500)
        truncated = portfolio_value <= max(self.initial_balance * 0.05, 500.0)
        
        # НОВЕ: Зберігаємо стан портфеля при завершенні епізоду для континуальності
        if (terminated or truncated) and self.enable_portfolio_continuity:
            ...
            # Додаємо поточну вартість портфеля до загальної історії
            current_portfolio_value = self._get_portfolio_value()
            self.cumulative_portfolio_history.append(current_portfolio_value)
            
            # Логування для відстеження прогресу
            total_return = (current_portfolio_value - self.initial_balance) / self.initial_balance
            print(f"📊 Episode {self.episode_count} завершено: Portfolio ${current_portfolio_value:.2f} ({total_return:+.1%})")
    ...
    def _execute_trade(self, trade_percentage: float) -> float:
        ...
        # 1. ПОКРАЩЕНЕ МАСШТАБУВАННЯ для збереження варіативності дій
        # Використовуємо tanh для м'якого обмеження замість жорсткого clipping
        trade_percentage = np.tanh(trade_percentage * 0.5) * 0.5  # Максимум ±50% позиція
        # НЕ використовуємо np.clip() щоб зберегти варіативність
        ...
        # 5. ЗМЕНШЕНИЙ ПОРІГ ДІЙ для збереження експлорації
        if abs(trade_percentage) < 0.005:  # 0.5% мінімальний поріг
            return 0.0
        ...
        # Перевіряємо мінімальну суму угоди - ЗНИЖЕНІ ЛІМІТИ для експлорації
        min_trade_amount = max(self.config.min_trade_amount, 25.0)  # Мінімум $25 для кращої активності
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        for col in normalized_data.columns:
            if col not in price_columns + ['volume']:
                # ВИПРАВЛЕННЯ: Використовуємо pandas.clip() замість numpy.clip() для Series
    ...
    def _get_info(self) -> Dict:
        ...
        # Win rate на основі завершених угод
        ...
        return {
            ...
            'total_return': episodic_return,  # Епізодна доходність
            ...
            # НОВІ: Cumulative метрики для портфельної континуальності
