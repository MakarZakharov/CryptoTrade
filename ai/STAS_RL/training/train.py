"""
Основний скрипт для навчання STAS_RL-агента.
"""
...
from CryptoTrade.ai.STAS_RL.environment.trading_env import TradingEnv
from CryptoTrade.ai.STAS_RL.agents.dqn_agent import DQNAgent
from CryptoTrade.ai.STAS_RL.agents.ppo_agent import PPOAgent
from CryptoTrade.ai.STAS_RL.training.callbacks import TradingCallback, TensorboardCallback


class DRLTrainer:
    """Клас для навчання STAS_RL агентів."""
    ...
    def create_callbacks(self, eval_freq: int = 10000, save_freq: int = 20000):
        """Створити callbacks для навчання - МАКСИМАЛЬНО ОПТИМІЗОВАНО для швидкості."""
        callbacks = []
        
        # Callback для оцінки моделі - МАКСИМАЛЬНО РІДКО для швидкості + НЕДЕТЕРМІНОВАНИЙ для експлорації
        eval_callback = EvalCallback(
            self.val_env,
            best_model_save_path=f"{self.save_dir}/{self.experiment_name}",
            log_path=f"logs/{self.experiment_name}",
            eval_freq=eval_freq * 4,  # ПОЧЕТВЕРЕНО інтервал оцінки для максимальної швидкості
            n_eval_episodes=1,  # МІНІМУМ епізодів для швидкості
            deterministic=False,  # КРИТИЧНО: Недетермінований для збереження експлорації
            render=False,
            verbose=1,
            warn=False
        )
        ...
        # Checkpoint callback - РІДШЕ збереження
        checkpoint_callback = CheckpointCallback(
            save_freq=save_freq * 2,  # ПОДВОЄНО інтервал збереження
        ...
        # Кастомні callbacks з рідкою звітністю
        trading_callback = TradingCallback(
            log_dir=f"logs/{self.experiment_name}",
            experiment_name=self.experiment_name
        )
        # ЗБІЛЬШЕНО інтервал звітності до 10000 кроків для швидкості
        trading_callback.report_interval = 10000
        callbacks.append(trading_callback)
        
        # Tensorboard callback - ОПТИМІЗОВАНО
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Навчання STAS_RL агента для торгівлі')
